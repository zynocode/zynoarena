...
// LudoGame and evaluateCPUMove are dynamically imported to reduce initial bundle size
...
// =========================================================================
export default function LudoGameView({ onExit }: { onExit: () => void }) {
  const { play } = useAudio();
  ...
  useEffect(() => {
    if (currentScreen === 'PLAYING' && !gameRef.current) {
      import('../../game/LudoGame').then(({ initLudoGame }) => {
        if (!gameRef.current && currentScreen === 'PLAYING') {
  ...
  // Turn Timer ticks
  useEffect(() => {
    if (currentScreen !== 'PLAYING' || gameStatus === 'GAME_OVER') return;
  ...
  }, [currentScreen, gameStatus, tickTimer]);
    ...
    if (gameStatus === 'WAITING_FOR_ROLL') {
      ...
      timeoutId = window.setTimeout(() => {
        if (validMoves.length > 0) {
          import('../../game/utils/ludoAI').then(({ evaluateCPUMove }) => {
  ...
  return (
    ...
    <div className="app-container">
      ...
      {/* LUDO UI */}
      {currentScreen === 'MENU' && <MainMenu onBackToArena={handleBackToArena} />}
      {currentScreen === 'SETUP' && <MainMenu onBackToArena={handleBackToArena} />}
