...
import { LogOut, Volume2, VolumeX, RotateCcw, Play, Trophy } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    if (currentScreen === 'GAME_OVER' && prevScreenRef.current !== 'GAME_OVER') {
      playSound('win', mute);
    }
  ...
  }, [currentScreen, activePlayerIndex, gameStatus, validMoves, players, rollDice, selectToken, diceValue, mute]);
  ...
  return (
    <div style={{ position: 'relative', width: '100%', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      {/* MENU / SETUP */}
      {(currentScreen === 'MENU' || currentScreen === 'SETUP') && <MainMenu />}
          ...
          <div className="game-board-col">
            ...
            <div className="glass-panel game-hud">
              ...
              <div style={{ display: 'flex', gap: '6px' }}>
                ...
                <button onClick={resetGame} title="Quit"
                  style={{ background: 'rgba(239,68,68,0.1)', border: 'none', borderRadius: '8px', padding: '7px', cursor: 'pointer', color: '#ef4444', display: 'flex', alignItems: 'center', gap: '4px', fontSize: '12px', fontWeight: 600 }}>
                  <LogOut size={14} /> Quit
                </button>
              </div>
            </div>
              ...
              {matchSettingsRef.current && (
                <button className="btn-primary" onClick={() => {
                  if (matchSettingsRef.current) {
                    setupGame(matchSettingsRef.current.numCPUs, matchSettingsRef.current.difficulty, matchSettingsRef.current.color);
                  }
                }} style={{ display: 'flex', alignItems: 'center', gap: '6px', flex: 1 }}>
                  <Play size={16} /> Play Again
                </button>
              )}
              <button className="btn-secondary" onClick={resetGame} style={{ display: 'flex', alignItems: 'center', gap: '6px', flex: 1 }}>
